...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "All image reads, Base64 encoding, and request JSON "
            "serialization are completed before warm-up and measurement."
        )
    )
...
def image_to_data_url(image_path: Path) -> str:
    """Read and Base64-encode an image before the measured request loop."""
...
def build_request_body(
    ...
    """Build one complete serialized HTTP request body.

    Image reading, Base64 encoding, Python dictionary construction, and JSON
    serialization all happen here during preprocessing. The measured request
    loop therefore sends already-prepared bytes.
    """
...
def prepare_evaluation_rows(
    ...
    """Resolve paths and build every serialized request before measurement."""
    ...
    print("Preparing image data and serialized request bodies...")
    ...
    for index, row in enumerate(selected_rows, start=1):
        ...
        try:
            ...
            raise OSError(
                f"Could not read or encode image {image_path}: {exc}"
            ) from exc
        ...
        total_request_bytes += len(request_body)

        if index == total or index % 10 == 0:
            print(f"  Prepared {index}/{total} images")

    elapsed_s = time.perf_counter() - start
    return prepared_rows, elapsed_s, total_request_bytes
...
def main() -> int:
    ...
    prepared_rows, preprocessing_s, total_request_bytes = (
    ...
    request_mib = total_request_bytes / (1024 * 1024)
    endpoint_url = (
    ...
    print(f"Preprocessing:   {preprocessing_s:.3f} s")
    print(f"Prepared bodies: {request_mib:.2f} MiB")
    print(
        "Measured wall time excludes image reads, Base64 encoding, "
    ...
    # Warm up the model, CUDA kernels, image encoder, and server slot using an
    # already-prepared body. Warm-up requests are excluded from the CSV.
    ...
    for warmup_index in range(args.warmup):
        ...
        request_prediction(
            ...
            request_body=warmup_row["request_body"],
    ...
    print(f"Preprocessing:  {preprocessing_s:.3f} s (excluded)")
